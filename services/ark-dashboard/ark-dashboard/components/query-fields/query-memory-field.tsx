...
export function QueryMemoryField({ mode, value, onChange, label, availableMemories, loading = false }: QueryMemoryFieldProps) {
  if (mode === 'view') {
    return (
      ...
      <td className="px-3 py-2">
        <Select
          ...
          onValueChange={(selectedValue) => {
            onChange?.(selectedValue === "__none__" ? undefined : { name: selectedValue })
          }}
          disabled={loading}
        >
          <SelectTrigger className="w-full h-9">
            <SelectValue className="text-sm" placeholder={loading ? "Loading..." : "Select memory (optional)"} />
          </SelectTrigger>
          <SelectContent>
