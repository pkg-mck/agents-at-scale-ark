...
export const useCreateSecret = (props: useCreateSecretProps) => {
  ...
  return useMutation({
    ...
    onMutate: async (newSecret) => {
      ...
      await queryClient.cancelQueries({ queryKey: [GET_ALL_SECRETS_QUERY_KEY] })
      ...
      queryClient.setQueryData(
        [GET_ALL_SECRETS_QUERY_KEY],
        (old: Secret[]): Secret[] => [...old, { id: newSecret.name, name: newSecret.name }]
      )
    ...
    onError: (error, data, onMutateResult) => {
      ...
      queryClient.setQueryData([GET_ALL_SECRETS_QUERY_KEY], onMutateResult?.previousTodos)
