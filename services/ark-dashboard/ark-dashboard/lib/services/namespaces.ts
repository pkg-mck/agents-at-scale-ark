...
export type Namespace = components['schemas']['NamespaceResponse'] & { id: number }
...
export const namespacesService = {
    ...
    async getAll(): Promise<Namespace[]> {
        const response = await apiClient.get<NamespaceListResponse>('/api/v1/namespaces')
    ...
    async create(name: string): Promise<Namespace> {
        ...
        const response = await apiClient.post<components['schemas']['NamespaceResponse']>('/api/v1/namespaces', request)
        return {
            ...response,
