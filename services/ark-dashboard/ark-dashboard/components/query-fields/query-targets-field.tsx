...
export function QueryTargetsField({ mode, value = [], onChange, label, availableTargets, loading = false }: QueryTargetsFieldProps) {
        ...
        <td className={FIELD_HEADING_STYLES}>
          <TooltipProvider>
            <Tooltip>
              ...
              <TooltipContent>
                <p>Specific agents, teams, models, or tools that will execute this query</p>
              </TooltipContent>
            </Tooltip>
          </TooltipProvider>
        </td>
        <td className="px-3 py-2 text-xs text-gray-700 dark:text-gray-300">
          {value.length > 0 ? value.map(t => `${t.type}:${t.name}`).join(", ") : "—"}
        </td>
      </tr>
    )
  }
        ...
        <div className="space-y-2">
          ...
          <DropdownMenu open={open} onOpenChange={setOpen}>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" className="w-full justify-between text-sm h-9 font-normal min-w-0" disabled={loading}>
            ...
            <DropdownMenuContent className="w-80" align="start">
              {loading ? (
                <div className="p-3 text-sm text-gray-500">Loading targets...</div>
              ) : (
                <>
                  {/* Filter input */}
                  <div className="p-2">
                  ...
                  <div className="max-h-64 overflow-auto">
                    {Object.entries(groupedTargets).map(([type, targets], index, array) => (
                      <div key={type}>
                        ...
                        <div className="px-2 pb-2">
                          {targets.map((target) => (
                            <div key={`${target.type}-${target.name}`} className="flex items-center space-x-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 rounded px-2">
                              <Checkbox
                                id={`${target.type}-${target.name}`}
                                checked={isTargetSelected(target)}
                                onCheckedChange={(checked) => handleTargetToggle(target, !!checked)}
                              />
                              <label
                    ...
                    {Object.keys(groupedTargets).length === 0 && (
                      <div className="p-3 text-sm text-gray-500">
                        {filter ? "No targets match your filter" : "No targets available"}
                      </div>
                    )}
                  </div>
                </>
