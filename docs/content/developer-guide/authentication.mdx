...
- **`sso`**: Full authentication required. Users must authenticate with the OIDC provider to access the dashboard.
- **`open`** (default): No authentication required. The application allows unrestricted access without signing in.
