...
graph TD
    ...
    subgraph "Selector Strategy"
        SEL_INPUT[Input] --> SEL_MODEL[Selector Model]
        SEL_MODEL --> SEL_A[Select Agent A]
        SEL_MODEL --> SEL_B[Select Agent B]
        SEL_MODEL --> SEL_C[Select Agent C]
    end
```
...
AI model chooses next participant based on conversation context.
...
- Uses AI model to select next participant
...
- **Selector**: `selector.model`, `selector.selectorPrompt`
