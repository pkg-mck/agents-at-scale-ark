...
# Check the status of the phoenix pods - they will take a few minutes to show
# as 'Ready' while the the database starts. Or explicitly wait for the
...
# If your ark controller is already installed/running, then restart it so that
# it picks up the updated OTEL configuration.
