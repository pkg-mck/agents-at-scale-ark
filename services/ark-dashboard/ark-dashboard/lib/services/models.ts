...
export const modelsService = {
  ...
  async getByName(name: string): Promise<Model | null> {
    try {
      const response = await apiClient.get<ModelDetailResponse>(`/api/v1/models/${name}`)
      return {
        ...response,
  ...
  async create(model: ModelCreateRequest): Promise<Model> {
    const response = await apiClient.post<ModelDetailResponse>(`/api/v1/models`, model)
    return {
      ...response,
      id: response.name
    }
  ...
  async update(name: string, updates: ModelUpdateRequest): Promise<Model | null> {
    try {
      const response = await apiClient.put<ModelDetailResponse>(`/api/v1/models/${name}`, updates)
      return {
        ...response,
        id: response.name
      }
  ...
  async updateById(id: number | string, updates: ModelUpdateRequest): Promise<Model | null> {
    const name = String(id)
