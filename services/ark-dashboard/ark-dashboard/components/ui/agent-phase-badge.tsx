...
export function getAgentPhase(agent: Agent): AgentPhase {
  if (!agent.status || typeof agent.status !== "object" || !("phase" in agent.status)) {
    return "unknown";
  }
 ...
 */
export function AgentPhaseBadge({ agent, className = "" }: AgentPhaseBadgeProps) {
  const phase = getAgentPhase(agent);
  const config = PHASE_CONFIG[phase];

  return (
    <span
      className={`inline-flex px-2 py-1 text-xs font-medium rounded-full ${config.className} ${className}`}
    >
      {config.text}
    </span>
  );
