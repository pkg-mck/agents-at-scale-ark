...
export default function ChatManager() {
  ...
  const pendingEventsRef = useRef<Array<{ type: 'opened' | 'closed', name: string }>>([])
    ...
    if (pendingEventsRef.current.length > 0) {
      ...
      events.forEach(event => {
        if (event.type === 'opened') {
          window.dispatchEvent(new CustomEvent('chat-opened', { detail: { name: event.name } }))
        } else {
          window.dispatchEvent(new CustomEvent('chat-closed', { detail: { name: event.name } }))
        }
      })
    }
  }, [chatWindows])
  ...
  useEffect(() => {
    ...
    window.addEventListener('open-floating-chat', handleOpenChat as EventListener)
    window.addEventListener('toggle-floating-chat', handleToggleChat as EventListener)
    return () => {
      window.removeEventListener('open-floating-chat', handleOpenChat as EventListener)
      window.removeEventListener('toggle-floating-chat', handleToggleChat as EventListener)
    }
  }, [])
  ...
  const handleCloseChat = (id: string) => {
    setChatWindows(prev => {
      ...
      if (closingChat) {
        // Queue event to be dispatched after state update
        pendingEventsRef.current.push({ type: 'closed', name: closingChat.name })
      }
